...
def kernel_TunstallDecode(
    ...
    for i_block in range(N // N_TILE):
        ...
        ops = op_decompressed + out_base + (cum_count - count)  # shape = (N_TILE, ), pointer
        out_base += tl.sum(count)
        for i in range(7):
            items >>= 8
            bytes_ = items.to(tl.int8)
            # Prefer count-based validity over sign-bit masking for backend portability.
            tl.store(ops, bytes_, mask=(i < count))
            ops += 1
